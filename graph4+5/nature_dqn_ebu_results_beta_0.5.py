...
class ExplorationExploitationScheduler(object):
    ...
    def get_action(self, session, frame_number, state, evaluation=False):
        ...
        if np.random.rand(1) < eps:
            _,q_values = session.run([self.DQN.best_action,self.DQN.q_values], feed_dict={self.DQN.input:[state]}) 
            action = np.random.randint(0, self.n_actions)
            return action,q_values[0][action]
        action,q_values = session.run([self.DQN.best_action,self.DQN.q_values], feed_dict={self.DQN.input:[state]})  
        q_values = q_values[0]
        action = action[0]
        ...
        while frame_number < MAX_FRAMES:
            ...
            while epoch_frame < EVAL_FREQUENCY:
                ...
                if len(rewards) % 10 == 0:
                    ...
                    with open('rewards_beta.dat', 'a') as reward_file:
            ...
            with open('mean_q_test_score_beta.csv', 'a') as q_reward_file:
            ...
            SUMM_WRITER.add_summary(summ, frame_number)
            with open('rewardsEval_beta.dat', 'a') as eval_reward_file:
...
if not TRAIN:
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            # sess.run(MAIN_DQN.q_values, feed_dict={EVAL_SCORE_PH:np.mean(eval_rewards)})
            # print(sess.eval(MAIN_DQN.q_values))
            # print()
            #print(np.mean(sess.run(explore_exploit_sched.DQN.q_values)))
            # print(np.mean(MAIN_DQN.q_values))
            if terminal == True:
                break
